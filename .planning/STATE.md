...
- [Phase B.7]: GetIdentityAndSignedPreKeyWithID returns spkUploadedAt as time.Time — buildBundle can call maybeSendSPKStale without extra DB round-trip
- [Phase B.7]: OPK count endpoint at /count mounted before /{userId}/{deviceId} to avoid chi route conflict; inherited 10 req/min rate limit is acceptable
- [Phase B.7]: PurgeConsumedOneTimePreKeys uses subquery LIMIT 10000 to bound DELETE scan time per background job invocation
- [Phase B.7]: SPK persist-before-upload ordering in keyMaintenance: new SPK written to IndexedDB before network upload so retry doesn't generate duplicate keypair
- [Phase B.7]: currentSPKId defaults to 1 matching initial uploadKeysAfterAuth ID; getNextSPKId defaults to 2 to avoid collision
- [Phase B.7]: OPK replenishment upload uses current SPK from IndexedDB, not freshly-generated bundle SPK, to prevent accidental SPK rotation during OPK upload
