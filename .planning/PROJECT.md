...
- ✓ Go backend with auth (register, login, guest, JWT sessions) — Phase A
...
- ✓ Auth UI migrated to Go backend (Matrix fully removed) — Phase E
...
- Cryptographic identity / challenge-response auth (Phase J) — post-MVP, current JWT placeholder is functional
- Guest access via capabilities (K.2) — post-MVP
- Instance discoverability / directory protocol (K.3) — post-MVP
- OAuth providers (Google, GitHub, Apple) — post-MVP
...
**Architecture model (from going_forward.md):** One backend = one community. No multi-tenancy. The client connects to N independent backends (desktop/mobile manage an instance list; web client is bound to URL). Each backend has its own user database, auth sessions, and Signal keys.
...
| JWT placeholder auth | Functional for MVP. Cryptographic identity (Phase J) replaces post-MVP | -- Pending |
