...
The MVP path forward: E+2 (multi-tenant restoration) → F (security hardening + moderation, guild-scoped) → K.1 (instance handshake) → K.4 (system messages per guild) → K.5 (default guild template) → B.7 (crypto key lifecycle) → J (BIP39 cryptographic identity + linked devices) → I (launch preparation).
...
**Success Criteria** (what must be TRUE):
  ...
  2. A session established using a superseded SPK (during the 7-day grace period) decrypts successfully; the server returns the correct historical private key during X3DH
...
**Execution Order:** E+2 → E+2.5 → F → G → K.1 → K.4 → K.5 → B.7 → J → I
