...
│   │   ├── authStorage.js        # JWT session token persistence (localStorage)
...
│   ├── auth/                    # JWT session tokens, signature verification (no passwords)
...
│   │
│   ├── contexts/
│   │   └── AuthContext.jsx      # Auth state (BIP39 identity + JWT session, no Matrix)
│   │
│   ├── hooks/
│   │   ├── useAuth.js           # Auth: BIP39 mnemonic → keypair, cryptographic auth, guest, session rehydration
...
5. **BIP39 mnemonic identity**: Every user's cryptographic identity derives from a 12-word BIP39 mnemonic. The mnemonic deterministically generates the root keypair. The server stores only public keys — no passwords, no email, no encrypted blobs. Authentication is cryptographic: the client proves ownership of the private key via a signature challenge. JWT is used for session tokens after authentication, not as the identity mechanism. Recovery relies solely on the 12 words; if lost with no linked devices, the account is irrecoverable.
...
8. **Hosted logic is isolated**: `hosted/` contains gethush.live-specific logic (managed instance provisioning, monitoring). The core app has zero payment or hosting awareness.
9. **Single Rust crypto crate**: `hush-crypto` wraps libsignal. Web uses WASM (via wasm-pack). Desktop (Tauri) uses the crate directly via Tauri commands. Mobile uses UniFFI bindings (Swift, Kotlin). One implementation, zero cross-platform interop risk.
...
**Key Exchange**:
1. On registration, user receives a BIP39 12-word mnemonic (shown once, irrecoverable if lost). The mnemonic deterministically derives the root identity key pair. The client also generates: signed pre-key, batch of one-time pre-keys
2. Root public key + Signal pre-keys uploaded to Go backend (`POST /api/keys/upload`). Server stores public keys only — no mnemonic, no password, no email
