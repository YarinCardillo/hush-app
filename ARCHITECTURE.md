...
9. **Single Rust crypto crate**: `hush-crypto` wraps OpenMLS. Web uses WASM (via wasm-pack). Desktop (Tauri) will use the crate directly via Tauri commands. Mobile will use UniFFI bindings (Swift, Kotlin). One implementation, zero cross-platform interop risk.
